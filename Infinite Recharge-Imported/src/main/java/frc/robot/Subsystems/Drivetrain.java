...
public class Drivetrain extends SubsystemBase implements SubsystemFramework {
    ...
    public static boolean LimelightOn(){
		return(Hardware.driverPad.getRawButton(Constants.LIMELIGHT));
	}
    @Override
    public void update() {
        ...
        switch(state) {
            case LowGear:
                ...
                // Joystick Control
                myRobot.arcadeDrive(Hardware.driverPad.getRawAxis(1) * 0.5, Hardware.driverPad.getRawAxis(4) * -0.5, false);
            ...
            case Auto:
                ...
                if (limelight.getLimelightHasValidTarget())
              {
                  myRobot.arcadeDrive(limelight.getLimelightDriveCommand(), limelight.getLimelightSteerCommand());
              }
                if(!LimelightOn()) {
                    newState = DrivetrainStates.HighGear;
                }
